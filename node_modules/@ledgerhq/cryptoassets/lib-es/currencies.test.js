...
import { listTokens, findTokenById, findTokenByAddressInCurrency, } from "./tokens";
import { listCryptoCurrencies, hasCryptoCurrencyId, getCryptoCurrencyById, findCryptoCurrency, findCryptoCurrencyById, findCryptoCurrencyByScheme, findCryptoCurrencyByTicker, findCryptoCurrencyByKeyword, registerCryptoCurrency, cryptocurrenciesById, } from "./currencies";
...
var currencies = Array.from(new Set(Object.values(cryptocurrenciesById)));
...
test("findTokenByAddressInCurrency", function () {
    expect(findTokenByAddressInCurrency("0x111111111117dC0aa78b770fA6A738034120C302", "bsc")).toMatchObject({
        id: "bsc/bep20/1inch_token"
    });
    expect(findTokenByAddressInCurrency("0x111111111117dC0aa78b770fA6A738034120C302", "ethereum")).toMatchObject({
        id: "ethereum/erc20/1inch_token"
    });
    expect(findTokenByAddressInCurrency("0x0", "bsc")).toBe(undefined);
    expect(findTokenByAddressInCurrency("0x111111111117dC0aa78b770fA6A738034120C302", "tron")).toBe(undefined);
});
...
test("testnet currencies must also set disableCountervalue to true", function () {
    expect(listCryptoCurrencies(true)
...
test("Ethereum family convention: all ethereum testnet coins must derivate on the same cointype as the testnet it's for (e.g. ethereum ropsten is on 60)", function () {
    expect(listCryptoCurrencies()
        .filter(function (e) {
        return e.family === "ethereum" && // ethereum family
            e.isTestnetFor && // is a testnet coin
            e.coinType !== getCryptoCurrencyById(e.isTestnetFor).coinType;
...
test("can register a new coin externally", function () {
    var coinId = "mycoin";
    expect(function () { return getCryptoCurrencyById("mycoin"); }).toThrow("currency with id \"".concat(coinId, "\" not found"));
    var mycoin = {
        type: "CryptoCurrency",
        id: coinId,
        coinType: 8008,
