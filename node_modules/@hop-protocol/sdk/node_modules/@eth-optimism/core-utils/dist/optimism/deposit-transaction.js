...
const { hexDataSlice, stripZeros, hexConcat, keccak256, zeroPad } = ethers_1.utils;
const formatBoolean = (value) => {
    return value ? new Uint8Array([1]) : new Uint8Array([]);
};
const formatNumber = (value, name) => {
    const result = stripZeros(ethers_1.BigNumber.from(value).toHexString());
    if (result.length > 32) {
...
class DepositTx {
        ...
        if (!this._sourceHash) {
            ...
            switch (this.domain) {
                case SourceHashDomain.UserDeposit:
                    marker = ethers_1.BigNumber.from(this.logIndex).toHexString();
                    break;
                case SourceHashDomain.L1InfoDeposit:
                    marker = ethers_1.BigNumber.from(this.sequenceNumber).toHexString();
                    break;
                default:
            ...
            const domain = ethers_1.BigNumber.from(this.domain).toHexString();
    ...
    encode() {
        const fields = [
            this.sourceHash() || '0x',
            ethers_1.utils.getAddress(this.from) || '0x',
            this.to != null ? ethers_1.utils.getAddress(this.to) : '0x',
        ...
        return hexConcat([
            ethers_1.BigNumber.from(this.type).toHexString(),
            ethers_1.utils.RLP.encode(fields),
        ]);
    }
    decode(raw, extra = {}) {
        const payload = ethers_1.utils.arrayify(raw);
        if (payload[0] !== this.type) {
            throw new Error(`Invalid type ${payload[0]}`);
        }
        this.version = payload[1];
        const transaction = ethers_1.utils.RLP.decode(payload.slice(1));
    ...
    fromL1Event(event) {
        ...
        let offset = 0;
        this.mint = ethers_1.BigNumber.from(hexDataSlice(opaqueData, offset, offset + 32));
        offset += 32;
        this.value = ethers_1.BigNumber.from(hexDataSlice(opaqueData, offset, offset + 32));
        offset += 32;
        this.gas = ethers_1.BigNumber.from(hexDataSlice(opaqueData, offset, offset + 8));
        offset += 8;
        const isCreation = ethers_1.BigNumber.from(opaqueData[offset]).eq(1);
        offset += 1;
