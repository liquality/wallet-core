...
describe("add external connection", () => {
    ...
    test('should be able validate externalConnections for multiple origins', async () => {
        ...
        expect(Object.keys(wallet.state.externalConnections[walletId]?.[originNameOne]?.ethereum).length)
            .toBeGreaterThan(0);
        expect(Object.keys(wallet.state.externalConnections[walletId]?.[originNameTwo]?.ethereum).length)
            .toBeGreaterThan(0);
