...
import { chains, currencyToUnit, unitToCurrency, ChainId, Asset } from '@liquality/cryptoassets'
...
class HopSwapProvider extends SwapProvider {
  ...
  async _getSigner(network: Network, walletId: string, from: string, fromAccountId: string, provider: ethers.providers.Provider) {
  ...
  async _getBridgeWithSigner(hopAsset: TToken, hopChainFrom: Chain, network: Network, walletId: string, from: string, fromAccountId: string) {
    try{
  ...
  _findAsset(asset: Asset, chain: string, tokens: Record<string, any>, tokenName: string) {
    if (asset.type === 'native') {
      ...
      if (
        asset.contractAddress && tokens[chain] &&
    ...
    try {
      ...
      if (!isFromL1Source) {
        ...
        const { data } = await clientGQL
          .query(getTransferIdByTxHash('0x' + fromFundHash))
          .toPromise()
   ...
   */
  // eslint-disable-next-line no-unused-vars
  async performNextSwapAction(store: ActionContext, { network, walletId, swap } : NextSwapActionRequest<HopSwapHistoryItem> ) {
    let updates
