    ...
    <div class="text-center">
      <table class="retry-button-table table bg-white border-0 mb-0 mt-0">
        <tbody
          ...
          v-if="item.type === 'SWAP' && item.error"
        >
          <tr>
      ...
      <table class="table bg-white border-0 mb-1 mt-1">
        <tbody class="font-weight-normal" v-if="item.type === 'SWAP'">
          ...
          <tr>
            ...
            <td id="swap_details_status" class="text-break">
              {{ item.status }}
            </td>
          </tr>
          <tr>
          ...
          <tr v-if="item.fromAddress">
            <td class="text-muted text-left small-12">
              Your {{ item.from }} address
            </td>
            <td id="from_address" class="text-break">
              <a
                :href="
                  addressLink(item.fromAddress, item.from, item.toAccountId)
                "
                target="_blank"
              ...
              <CopyIcon
                class="copy-icon"
                @click="copy(addPrefix(item.fromAddress, item.from))"
              />
            </td>
          </tr>
          <tr v-if="item.toAddress">
            <td class="text-muted text-left small-12">
              Your {{ item.to }} address
            </td>
            <td id="to_address" class="text-break">
              ...
              <CopyIcon
                class="copy-icon"
                @click="copy(addPrefix(item.toAddress, item.to))"
              />
            </td>
          </tr>
          <tr v-if="item.secret">
          ...
          <tr v-if="item.fromFundHash">
            <td class="text-muted text-left small-12">
              Your {{ item.from }} funding transaction
            </td>
          ...
          <tr v-if="item.toFundHash">
            <td class="text-muted text-left small-12">
              Counter-party's {{ item.bridgeAsset || item.to }} funding
              transaction
            </td>
          ...
          <tr v-if="item.approveTxHash">
            <td class="text-muted text-left small-12">
              Your {{ item.from }} approve transaction
            </td>
            <td id="approve_transaction" class="text-break">
              {{ item.approveTxHash }}
            </td>
          </tr>
          ...
          <tr v-if="item.receiveTxHash">
            <td class="text-muted text-left small-12">
              Your {{ item.to }} receive transaction
            </td>
            <td id="receive_transaction" class="text-break">
              {{ item.receiveTxHash }}
            </td>
          </tr>
          <tr v-if="item.sendTx">
            <td class="text-muted text-left small-12">
              Your {{ item.to }} send transaction
            </td>
            <td id="send_transaction" class="text-break">{{ item.sendTx }}</td>
          </tr>
          <tr v-if="item.refundHash">
            <td class="text-muted text-left small-12">
              Your {{ item.from }} refund transaction
            </td>
...
export default {
  ...
  methods: {
    ...
    getToAssetWhenSwappingFromNative() {
      if (
        this.item.provider.includes('liqualityBoost') &&
        !isERC20(this.item.from)
      ) {
