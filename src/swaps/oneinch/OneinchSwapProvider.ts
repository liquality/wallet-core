...
class OneinchSwapProvider extends SwapProvider {
  ...
  async _getQuote(chainIdFrom, from, to, amount) {
    ...
    const referrerAddress =
      this.config.referrerAddress?.[cryptoassets[from].chain];
  ...
  async getQuote({ network, from, to, amount }) {
    ...
    const chainIdFrom =
      ChainNetworks[cryptoassets[from].chain][network].chainId;
    const chainIdTo = ChainNetworks[cryptoassets[to].chain][network].chainId;
    if (chainIdFrom !== chainIdTo || !chainToRpcProviders[chainIdFrom])
      return null;

    const trade = await this._getQuote(
      chainIdFrom,
      from,
      to,
  ...
  async sendSwap({ network, walletId, quote }) {
    ...
    const swapParams: any = {
      ...
      toTokenAddress:
        cryptoassets[quote.to].contractAddress || nativeAssetAddress,
      amount: quote.fromAmount,
      fromAddress: fromAddress,
      slippage: quote.slippagePercentage
        ? quote.slippagePercentage
        : slippagePercentage,
    };

    const referrerAddress =
      this.config.referrerAddress?.[cryptoassets[quote.from].chain];
    if (referrerAddress) {
  ...
  async estimateFees({ network, txType, quote, feePrices }) {
    ...
    if (txType in OneinchSwapProvider.txTypes) {
      const fees = {};
      const tradeData = await this._getQuote(
        chainId,
        quote.from,
        quote.to,
        quote.fromAmount
      ...
      for (const feePrice of feePrices) {
        ...
        const fee = new BN(tradeData.data?.estimatedGas)
          .times(1.5)
          .times(gasPrice);
