...
test('get bitcoin derivation path for default account type Segwit', () => {
  expect(
    getDerivationPath(ChainId.Bitcoin, 'mainnet', 0, AccountType.Default)
  ).toBe("84'/0'/0'");
});
...
test('get ETH derivation path for default accounts on mainnet and testnet', () => {
  const ethPathMainnet = getDerivationPath(
    ChainId.Ethereum,
    'mainnet',
    0,
    AccountType.Default
  );
  const ethPathTestnet = getDerivationPath(
    ChainId.Ethereum,
    'testnet',
    0,
    AccountType.Default
...
test('get RSK derivation path for Ledger accounts on mainnet', () => {
  expect(
    getDerivationPath(ChainId.Rootstock, 'mainnet', 0, AccountType.RskLedger)
  ).toBe("m/44'/137'/0'/0/0");
});

test('get RSK derivation path for Ledger accounts on testnet', () => {
  expect(
    getDerivationPath(ChainId.Rootstock, 'testnet', 0, AccountType.RskLedger)
  ).toBe("m/44'/37310'/0'/0/0");
});

test('get RSK derivation path for default accounts on mainnet', () => {
  const ethPath = getDerivationPath(
    ChainId.Ethereum,
    'mainnet',
    0,
    AccountType.Default
  ...
  expect(
    getDerivationPath(ChainId.Rootstock, 'mainnet', 0, AccountType.Default)
  ).toBe("m/44'/60'/0'/0/0");
...
test('get RSK derivation path for default accounts on testnet', () => {
  ...
  expect(
    getDerivationPath(ChainId.Rootstock, 'testnet', 0, AccountType.Default)
  ).toBe("m/44'/60'/0'/0/0");
  expect(ethPath).toEqual(ethPath);
});
