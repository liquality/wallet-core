...
import { AssetTypes, ChainId, currencyToUnit, getChainByChainId, unitToCurrency } from '@liquality/cryptoassets';
...
class SovrynSwapProvider extends SwapProvider {
  ...
  async requiresApproval({ network, walletId, quote }: SwapRequest<SovrynSwapHistoryItem>) {
    ...
    const fromAddress = getChainByChainId(network, fromInfo.chain).formatAddress(fromAddressRaw);
  ...
  async estimateFees({
    ...
    const nativeAsset = getChainByChainId(network, cryptoassets[asset].chain).nativeAsset[0].code;
