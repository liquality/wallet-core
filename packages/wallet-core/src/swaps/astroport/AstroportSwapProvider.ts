...
class AstroportSwapProvider extends SwapProvider {
  ...
  async newSwap({ network, walletId, quote }: SwapRequest<AstroportSwapHistoryItem>) {
    ...
    if (isFromNative) {
      if (!denom) {
        throw wrapCustomError(CUSTOM_ERRORS.NotFound.AstroPortDenom);
      }
  ...
  async _getSwapRate(fromAmount: string, fromInfo: IAsset, toInfo: IAsset) {
    ...
    if (nativeToNative) {
      ...
      if (!fromTokenAddress || !toTokenAddress) {
        throw wrapCustomError(CUSTOM_ERRORS.NotFound.ContractAddress);
      }
      ...
      throw wrapCustomError(CUSTOM_ERRORS.Invalid.AstroPortSwapPair(fromInfo.type, toInfo.type));
    }
